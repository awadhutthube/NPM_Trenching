...
def generate_histogram(datapoints, number, start = -0.03, end = 0.08, interval_size = 0.0005, draw = False):
    ...
    if draw:    
        ...
        plt.savefig('../histograms/bag_4/frame_' + str(number))
    else:
...
def read_rosbag(filepath):
    ...
    for idx, (topic, msg, t) in enumerate(bag.read_messages(topics=['/sensor/side_camera/depth/color/points'])):
        ...
        print("Trench threshold is {}".format(trench_thresh))
        # mask_height(transformed_xyz, trench_thresh)
        heightmap = generate_heightmap(transformed_xyz, trench_thresh, mask = True)
        visualize_heightmap(heightmap, idx)        
...
def generate_heightmap(points, threshold, dim = 500, mask = False):
    ...
    # threshold -= min_val
    print(threshold)
    publish_threshold_frame(threshold)
...
# def mask_height(heightmap, threshold):
#     masked_points = points[points[:,2] < threshold-0.01]
#     print("Shape of masked points {}".format(masked_points.shape))
#     # masked_points = masked_points[masked_points[:,2] > threshold-0.001]
#     print(masked_points.shape)
#     masked_map = generate_heightmap(masked_points)
#     plt.figure(3)
#     plt.clf()
#     plt.imshow(masked_map, cmap = 'gray')
#     plt.pause(0.0002)
#     # visualize_heightmap(masked_map)
